...
import { getCustomerList, callApi, login } from "../middleware/api";
import {
  listCustomers,
  getCustomer,
  deleteCustomer,
...
export const thunkAuth = (
  ...
  response = data;
  if (type == SIGN_IN) {
    response = login(endpoint, method, data).catch(error=>{console.log(error)});
...
function getNewEntity(newAction) {
  switch (newAction) {
    case NEW_CUSTOMER:
      return {
...
async function callEndPoint(apiAction) {
  let response;
  const { type, endpoint, method, data, filters } = apiAction;
  if (!isNewAction(type)) {
    response = await callApi(endpoint, method, data, filters);
...
const isNewAction = (x) => x.toString().startsWith("NEW_");
